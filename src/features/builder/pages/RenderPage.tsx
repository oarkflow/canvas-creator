...
export default function RenderPage() {
  ...
  const handleRender = () => {
    setError(null);
    try {
      const parsed = parseExportedJSON(jsonInput);
      if (parsed) {
        setComponents(parsed.components);
        setIsRendering(true);
      } else {
        // Try parsing as raw components array
        const rawComponents = JSON.parse(jsonInput);
        if (Array.isArray(rawComponents)) {
          setComponents(rawComponents);
          setIsRendering(true);
        } else if (rawComponents.components) {
          setComponents(rawComponents.components);
          setIsRendering(true);
        } else {
          setError('Invalid JSON format. Expected an array of components or exported page JSON.');
        }
      }
    } catch (e) {
  ...
  const handleLoadFromBuilder = () => {
    ...
    try {
      ...
      setError('No saved page found in builder.');
    }
  };
      ...
      <div className="container mx-auto px-4 py-6">
        {!isRendering ? (
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <Card>
              ...
              <CardContent className="space-y-4">
                ...
                <Button onClick={handleRender} className="w-full" disabled={!jsonInput.trim()}>
          ...
          <div className="bg-card border border-border rounded-lg min-h-[600px] p-6">
            <ComponentRenderer components={components} dataSources={dataSources} />
          </div>
        )}
      </div>
    </div>
